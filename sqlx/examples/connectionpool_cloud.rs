...
async fn main() {
    ...
    let pool = PgPoolOptions::new().min_connections(5).max_connections(5).connect(&format!("postgres://{}:{}@{}:{}/{}?ssl=true&sslmode=verify-full&sslrootcert={}", username, password, hostname, port, database, ca_certificate_file)).await.expect("Error connecting to database");

    let rows = sqlx::query("select table_name from information_schema.tables where table_type = 'BASE TABLE'").fetch_all(&pool).await.expect("Error executing query");
